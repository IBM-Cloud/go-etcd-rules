...
func (bw *baseWorker) doWork(loggerPtr **zap.Logger,
	...
	var l lock.RuleLock
	var lockErr error
	for i := range ruleOpts.watcherTries {
		// Wait after first attempt
		if i > 0 {
			time.Sleep(ruleOpts.watcherWait)
		}
		// Attempt to get the rule lock
		l, lockErr = bw.locker.Lock(lockKey, lock.PatternForLock(metricsInfo.keyPattern), lock.MethodForLock("worker_lock"), lock.AttempForLock(i+1))
		if lockErr != nil {
			logger.Warn("Failed to acquire rule lock", zap.Error(lockErr), zap.Uint("attempt", i+1))
			if source != "watcher" {
				return
			}
		}
